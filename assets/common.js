...
(function(){
  const overlay = document.getElementById('enqModal');
  if(!overlay) return;

  window.openEnquiry = function(service){
    ...
    if(service){
      ...
      if(sel){
        for(let i=0;i<sel.options.length;i++){
          if(sel.options[i].text===service||sel.options[i].value===service){sel.selectedIndex=i;break;}
        }
      }
    }
  };
  ...
  window.submitEnquiry = function(e){
    ...
    const orig = '<span>Send Enquiry</span><svg viewBox="0 0 24 24" width="16" fill="white" style="flex-shrink:0"><path d="M22 2L11 13M22 2L15 22l-4-9-9-4 20-7z"/></svg>';

    btn.innerHTML = '<span>Sending…</span>';
    btn.disabled  = true;

    fetch(window._enqApi || 'api/send-enquiry.php', {method:'POST', body: new FormData(form)})
      .then(r => r.json())
      .then(data => {
        if(data.success){
          btn.innerHTML = '<span>✓ Enquiry Sent! We\'ll be in touch soon.</span>';
          btn.style.background = 'linear-gradient(135deg,#06d6a0,#0ea5e9)';
          form.reset();
          setTimeout(()=>{
            closeEnquiry();
            setTimeout(()=>{ btn.innerHTML=orig; btn.style.background=''; btn.disabled=false; },400);
          },2400);
        } else {
  ...
  overlay.addEventListener('click',function(e){if(e.target===this)closeEnquiry();});

  // Close on Escape key
  document.addEventListener('keydown',function(e){
    if(e.key==='Escape'&&overlay.classList.contains('open'))closeEnquiry();
  });
})();