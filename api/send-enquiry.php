...
$rateFile = sys_get_temp_dir() . '/adt_enquiry_' . md5($_SERVER['REMOTE_ADDR'] ?? '');
$window = 600; $limit = 3;
...
if (!$name || !$email || !$message) json(false, 'Please fill in all required fields.');
...
$html = buildEmailHtml($subject, 'Quick Enquiry', $fields, $message);
...
try {
    ...
    $mail->send();
    json(true, 'Enquiry sent! Our team will reach you within 24 hours.');
} catch (Exception $e) {
